...
import random
import numpy as np
from zbot5_model import get_actions
...
while True:
    ...
    squares = []
    board = np.zeros((game_map.height*2, game_map.width*2, 4))
        ...
        for x in range(game_map.width):
            ...
            for dx in [1, 2]:
                for dy in [1, 2]:
                    ...
                    if (square.x, square.y) in prev_moves:
                        board[square.y*dy,square.x*dx,3] = prev_moves[(square.x, square.y)] / 4.0
    MAX_SQUARES = 256
